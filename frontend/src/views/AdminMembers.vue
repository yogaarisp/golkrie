  ...
  <AdminLayout>
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
      <div>
        ...
        <p class="text-on-surface-variant text-sm">Kelola daftar semua pemain Golkrie.</p>
      </div>
      <button @click="openModal()" class="w-full md:w-auto bg-primary-container text-on-primary-container px-6 py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:scale-105 transition-all">
    ...
    <div v-else class="space-y-4">
      ...
      <div class="md:hidden grid grid-cols-1 gap-4">
        <div v-for="member in members" :key="member.id" class="glass-card p-4 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold text-sm">
              {{ member.full_name.charAt(0) }}
            </div>
            <div>
              <div class="text-white font-bold">{{ member.full_name }}</div>
              <div class="text-[10px] text-on-surface-variant font-mono">{{ member.phone_number || '-' }}</div>
            </div>
          </div>
          <div class="flex gap-1">
            <button @click="openModal(member)" class="p-2 text-blue-400"><span class="material-symbols-outlined text-xl">edit</span></button>
            <button @click="deleteMember(member.id)" class="p-2 text-red-400"><span class="material-symbols-outlined text-xl">delete</span></button>
          </div>
        </div>
      </div>

      <div v-if="members.length === 0" class="glass-card px-6 py-12 text-center text-on-surface-variant">
        Belum ada member.
      </div>
    </div>
